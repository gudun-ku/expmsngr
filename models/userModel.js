...
userSchema.pre('save', async function(next) {
  ...
  const prePass = await crypto
    .createHash('SHA1')
    .update(this.password)
    .digest('hex');
  //console.log(prePass);
  // hash password or encrypt it by bcrypt with cost of 12
  this.password = await bcrypt.hash(prePass, 12);
  // not to save in db
...
userSchema.methods.correctPassword = async function(
  ...
  const prePass = await crypto
    ...
    .digest('hex');
  return await bcrypt.compare(prePass, userPassword);
};

userSchema.methods.correctLoyaPassword = async function(
  candidatePassword,
  userPassword
) {
